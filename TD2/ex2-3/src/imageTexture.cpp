...
ImageTex::ImageTex(const ImageTex* texsource,bool copie) {
	...
	switch (typetext) {
		...
		case (TEX_TYPE_RVBA) :
			...
			break;
		default :
			cerr<<"[VBVector3D : ImageTex] : Chargement d'un mauvais type "<<typetext<<" dans constructeur"<<endl;
			exit(5);
	}
	if (decal > 0 && copie) {
		tabRVB = new unsigned char[tailu*tailv*decal];
		memcpy(tabRVB,texsource->getTab(),tailu*tailv*decal);
	}
	else {
		tabRVB = NULL;
	}
...
ImageTex::ImageTex(int indexin,int type,int tailuin,int tailvin,bool reserve) {
	...
	if (decal > 0 && reserve) {
		...
		memset(tabRVB,0,tailu*tailv*decal);
	}
	else {
		tabRVB = NULL;
	}
...
ImageTex::ImageTex(int indexin,int type,int tailuin,int tailvin,unsigned char* imag_src) {
	...
	if (decal > 0) {
		...
		memcpy(tabRVB,imag_src,tailu*tailv*decal);
	}
}
...
void ImageTex::makePowerOfTwo(int& new_w,int& new_h,int flag_closer) {
	...
	switch (typetext) {
		...
		case(TEX_TYPE_NULL) :
		default :
			cerr<<"[VBVector3D] : Mauvais type de texture (ou indéfini) ! Unscalable !"<<endl;
	}
...
void ImageTex::lecturePpm(FILE *f) {
	...
	if (strcmp(tamp,"P6") && strcmp(tamp,"P5")) {
		cerr << "[VBVector3D] Lecture PPM : Fichier de texture not a .ppm";
		exit(5);
	}
	...
	if (flagp5) {
		...
		tabRVB = new unsigned char[tailu*tailv];
		if (tabRVB == NULL) {
			cerr<<"[VBVector3D] Lecture PPM : Probleme d'allocation memoire ImageTex::lecturePpm !"<<endl;
			exit(5);
		}
		for(int i=0;i<tailv;i++) {
			cpttmp = fread(tabRVB+(tailv-i-1)*tailu,sizeof(unsigned char),tailu,f);
		}
	}
	else {
		tabRVB = new unsigned char[tailu*tailv*3];
		if (tabRVB == NULL) {
			cerr<<"[VBVector3D] Lecture PPM : Probleme d'allocation memoire ImageTex::lecturePpm !"<<endl;
			exit(5);
		}
		for(int i=0;i<tailv;i++) {
			cpttmp = fread(tabRVB+(tailv-i-1)*3*tailu,sizeof(unsigned char),tailu*3,f);
		}
	}
}
...
void ImageTex::lectureRgba(int sizex,int sizey,FILE* f) {
	...
	tailu = sizex;
	tailv = sizey;
	unsigned char* tst = new unsigned char[tailu*tailv*4];
	if (!tst) {
		cerr<<"[VBVector3D] Lecture RGBA : Pb allocation memoire !"<<endl;
		exit(5);
	}
	cpttmp = fread((char*)tst,sizeof(unsigned char),tailu*tailv*4,f);

	tabRVB =  new unsigned char[tailu*tailv*4];
	if (!tabRVB) {
		cerr<<"[VBVector3D] Lecture RGBA : Pb allocation memoire !"<<endl;
		exit(5);
	}
...
void ImageTex::lectureTgaNoCompress(FILE *file,int d) {
  ...
  unsigned int size = nbpixel * tailu * tailv;
  ...
  for(cswap = 0; cswap < size; cswap += nbpixel)
...
void ImageTex::lectureTgaCompress(FILE *file,int d) {
  ...
  if(bpp == 4) {
  	//cerr<<"Type = TYPE RVBA"<<endl;
  	typetext = TEX_TYPE_RVBA;
  }
  else {
  	//cerr<<"Type = TYPE RVB"<<endl;
  	typetext = TEX_TYPE_RVB;
  }
