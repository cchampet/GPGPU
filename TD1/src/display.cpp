...
void drawScene(void) {
	...
	switch(which_test) {
		...
		case (5) :
			...
			break;
		case (6) :
			if (flag_shaders) makeConvolution3x3GPU(image_base,image_gpu);
			else  makeConvolution3x3(image_base,image_cpu);
			break;
		case (7) :
			if (flag_shaders) makeConvolution5x5GPU(image_base,image_gpu);
			else  makeConvolution5x5(image_base,image_cpu);
			break;
		default :
